<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit
      ...
      <el-table-column align="center" label="适配正则" prop="isRe"></el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="tempScriptConfig" label-position="left" label-width="80px"
        ...
        <el-form-item label="正则处理"  required>
          <el-select v-model="tempScriptConfig.isRe"  placeholder="请选择" style='width: 220px;'> <!-- 对应列名 clearable 清空当前checkbox-->
            <el-option
              v-for="item in allValue"
              :key="item.dailyReCode"
              :label="item.dailyRe"
              :value="item.dailyRe">
            </el-option>
          </el-select>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAllReValue() {
        this.api({
          url: "/scriptConfig/getAllRe",
          method: "get"
        }).then(data => {
          this.allValue = data.list;
        })
      },
      getList() {
        //查询列表
      ...
      showUpdate($index) {
        ...
        this.tempScriptConfig.isRe          = shell.isRe;
